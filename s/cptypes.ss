...
(let ()
  ...
  (module (dict-empty dict-ref dict-set dict-merge dict-merge/skip)
    (include "intmap.ss")
    ...
    (define (dict-merge/skip left right skipped)
      ($intmap-merge fx=
		     select
		     identity
		     identity
		     ...
		     left
		     right))
;      (let loop ([result (dict-merge left right)] [xs skipped])
;	(cond [(null? xs) result]
;	      [else (loop (dict-remove result (car xs)) (cdr xs))])))

    (define (select k v1 v2 nil)
      (cond
  ...
  (module (pred-env-empty pred-env-add pred-env-merge pred-env-merge/skip pred-env-lookup)
    ...
    (define pred-env-merge/skip dict-merge/skip)
                    ...
                    [(case-lambda ,preinfo (clause (,x* ...) ,interface ,body))
                     ...
                     (let ([subtypes (box (unbox types))])
                       ...
                       (let* ([t-subtypes (and t-types (box #f))]
                         ...
                         (set-box! types (pred-env-merge/skip (unbox types) (unbox subtypes) x*))
                         (when t-types
                           (unless (eq? (unbox t-subtypes) (unbox subtypes))
                             (set-box! t-types (pred-env-merge/skip (unbox types) (unbox t-subtypes) x*))))
                         (when f-types
                           (unless (eq? (unbox f-subtypes) (unbox subtypes))
                             (set-box! f-types (pred-env-merge/skip (unbox types) (unbox f-subtypes) x*))))
                         `(case-lambda ,preinfo (clause (,x* ...) ,interface ,body))))]
                    [(case-lambda ,preinfo ,cl* ...)
